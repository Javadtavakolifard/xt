...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'ریمو' and is_momod(msg) then
			local receiver = channel..matches[3]
			local user = "user#id"..matches[2]
			chaannel_kick(receiver, user, ok_cb, false)
		end

			if matches[1] == 'ادمین کن' then
		...
		if matches[1] == 'انتخاب ادمین اصلی' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
		...
		if matches[1] == 'لغو قفل' and is_momod(msg) then
			local target = msg.to.id
			     if matche[2] == 'همه' then
      	local dsafemode ={
        unlock_group_links(msg, data, target),
		...
		if matches[1] == 'لغو سکوت' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'صدا' then
			local msg_type = 'Audio'
		...
		if matches[1] == "سایلنت" or matches[1] == "لغو سایلنت" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
		...
		if matches[1] == 'دستورات' and not is_owner(msg) then
			text = "Message /superhelp to @shahin_xtbot in private for SuperGroup help"
