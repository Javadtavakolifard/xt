...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'پاک' and is_admin1(msg) and not matches[2] then
		...
		if matches[1] == "رباتها" and is_momod(msg) then
			member_type = 'Bots'
		...
		if matches[1] == "ریموو" and is_momod(msg) then
		...
		if matches[1] == 'بلاک' or matches[1] == 'ریمو' and is_momod(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
				...
				get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'بلاک' or matches[1] == 'kick' and string.match(matches[2], '^%d+$') then
		...
		return "💠اطلاعات و مشخصات شما \n\n  👤 نام: : " ..string.gsub(msg.from.print_name, "_", " ").. "\n 🆔 نام کاربری: : @"..(msg.from.username or '----').."\n ℹ️ شناسه: "..msg.from.id.."\n\nمشخصات گروه:\n 🌐 نام گروه: " ..string.gsub(msg.to.print_name, "_", " ").. "\n 🔰شناسه گروه: "..msg.to.id   end
		end
		...
		if matches[1] == 'تعویض لینک' and is_owner(msg) then
		...
		if matches[1] == 'مشخصه' and is_owner(msg) then
			local cbres_extra = {
				channelid = msg.to.id,
				get_cmd = 'res'
			}
		...
		if matches[1] == 'ن ادمین' then
			...
			elseif matches[1] == 'ن ادمین' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'demoteadmin'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'ن ادمین' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'demoteadmin'
				}
		...
		if matches[1] == 'مدیر اصلی' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
			...
			elseif matches[1] == 'مدیر اصلی' and string.match(matches[2], '^%d+$') then
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
				...
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'مدیر اصلی' and not string.match(matches[2], '^%d+$') then
				local	get_cmd = 'setowner'
				local	msg = msg
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'کاربر' then
			...
			elseif matches[1] == 'کاربر' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
		...
		if matches[1] == "اسم گروه" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == "توضیحات گروه" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == "تنظیم ایدی" and is_admin1(msg) then
		...
		if matches[1] == 'قوانین گروه' and is_momod(msg) then
			rules = matches[2]
			local target = msg.to.id
		...
		if matches[1] == 'ازاد' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'صدا' then
			local msg_type = 'Audio'
		...
		if matches[1] == 'دستورات' and not is_owner(msg) then
			text = "Message /superhelp to @XT_ROBO in private for SuperGroup help"
			reply_msg(msg.id, text, ok_cb, false)
		elseif matches[1] == 'دستورات' and is_owner(msg) then
